...
/*
classe que representa o computador no nivel dificil
 */
public class Computador3 extends Jogador {
    ...
    private int posicao = 8;

    // construtor
    public Computador3(boolean turno, char simbolo) {
        super(turno, simbolo);
    ...
    public void Jogar2(char[][] tab, char simbolo) {
        // segue dando preferencia a linha porem de baixo pra cima
        verificarposicao(posicao);
        if (String.valueOf(tab[this.getX()][this.getY()]).equals(" ")) {
            tab[this.getX()][this.getY()] = simbolo;
            this.setTurno(false);
        } else {
            posicao--;
            Jogar2(tab, simbolo);
